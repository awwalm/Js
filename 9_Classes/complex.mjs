 ...
 * Recall that a complex number is the sum of a real number
 * and an imaginary number and that imaginary number i
 * is the square root of -1.
 */
class Complex {
    // Once class field declarations are standardized, 
    // we could declare private fields to hold the real 
    // and imaginary parts of a complex number here:
    // #r = 0;
    // #i = 0;

    /**
     * This constructor function defines the instance fields `r` and `i`
     * on every instance it creates. These fields hold the real and imaginary
    ...
    constructor(real, imaginary) {
     ...
     // we might write c.plus(d) or d.times(c)
    plus(that) {
    ...
    // would be useful if we were using private fields this.#r & this.#i
    ...
    // Once static fields are supported inside class bodies,
    // we could define a useful Complex.ZERO constant like this:
...
// With the Complex class defined, we can use the constructor, 
// instance fields, instance methods, class fields, and class methods 
// with code like this:
let c = new Complex(2,3);       // Create a new object with the constructor
let d = new Complex(c.i, c.r);  // Use instance fields of c
let s = c.plus(d).toString();   // => "{5,5}"
console.log(s);
console.log(c.magnitude);       // => Math.hypot(2,3); Use a getter function
let p = Complex.product(c,d);   // => new Complex(0,13); A static method
console.log(p);
