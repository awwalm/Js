...
// This class extends Map so that they get() method returns the specified
...
class DefaultMap extends Map {
    ...
    get(key) {
        if (this.has(key)) {                    // If they key is already in the map
...
class Histogram {
    ...
    // Convert the histogram to a string that displays ASCII graphics
    toString() {
        // Convert the Map to an array of [key, value] arrays
        // `entries = [[k,v] for k,v in letterCounts]` in Python 
...
async function histogramFromStdin() {
    process.stdin.setEncoding("utf-8");                 // Read Unicode strings, not byte
